...
#' @param lamdba Computational smoothing parameter. This value is weighted by n to form the penalty coefficient (see Details). Ignored if spar is provided.
#'
...
fitSpline = function(x, spar = numeric(), m = 2, nknots = .nknots.smspl, all.knots = FALSE, lambda = NULL) {
  fit = ss(x = x$phys, y = x$gen, spar = spar, m = 2, nknots = .nknots.smspl, all.knots = FALSE, lambda = NULL)
  return(fit)
}
